    ...
    internal static class CVRMAObjectTogglePass
    {
        ...
        internal static void Run(GameObject avatarRoot)
        {
            ...
            foreach (var toggle in toggles)
            {
                ...
                if (controller != null)
                {
                    var merger = toggle.gameObject.AddComponent<CVRMAMergeAnimator>();
                    merger.animator              = controller;
                    merger.pathMode              = CVRMAPathMode.Relative;
        ...
        private static AnimatorController BuildToggleController(GameObject avatarRoot, CVRMAObjectToggle toggle)
        {
            ...
            ctrl.AddParameter(machineName, AnimatorControllerParameterType.Bool);
            ...
            sm.defaultState = toggle.defaultValue ? onState : offState;
            ...
            var ctrlPath = $"{TempFolder}/MA_CVR_Toggle_{machineName}_{GUID.Generate()}.controller";
            AssetDatabase.CreateAsset(ctrl, ctrlPath);

            AssetDatabase.AddObjectToAsset(sm,      ctrl); sm.hideFlags      = HideFlags.HideInHierarchy;
            AssetDatabase.AddObjectToAsset(onClip,  ctrl); onClip.hideFlags  = HideFlags.HideInHierarchy;
            AssetDatabase.AddObjectToAsset(offClip, ctrl); offClip.hideFlags = HideFlags.HideInHierarchy;
