...
namespace ModularAvatarCVR.Editor
{
    internal static class CVRMAMergeAnimatorPass
    {
        ...
        {
            // Try the current override controller's base — but skip if it's one of our
            // own temp assets from a previous build (which will have been deleted).
            if (avatar.overrides != null)
            {
                var existing = avatar.overrides.runtimeAnimatorController as AnimatorController;
                if (existing != null)
                {
                    var existingPath = AssetDatabase.GetAssetPath(existing);
                    bool isTemp = string.IsNullOrEmpty(existingPath) ||
                                  existingPath.StartsWith(TempFolder, System.StringComparison.OrdinalIgnoreCase);
                    if (!isTemp)
                        return DuplicateController(existing);
                }
            }

            // Fall back to the AAS base controller (editor-only but may be set by user).
            if (avatar.avatarSettings?.baseController is AnimatorController baseCtrl && baseCtrl != null)
            {
                var basePath = AssetDatabase.GetAssetPath(baseCtrl);
                bool isTemp = string.IsNullOrEmpty(basePath) ||
                              basePath.StartsWith(TempFolder, System.StringComparison.OrdinalIgnoreCase);
                if (!isTemp)
                    return DuplicateController(baseCtrl);
            }

            // Create a fresh empty controller as a last resort.
        ...
        {
            // The merged AnimatorController must be saved as an asset so the
            // AnimatorOverrideController below can reference it persistently.
            var ctrlPath = AssetDatabase.GetAssetPath(controller);
            if (string.IsNullOrEmpty(ctrlPath))
            {
                ctrlPath = $"{TempFolder}/MA_CVR_Merged_{GUID.Generate()}.controller";
                AssetDatabase.CreateAsset(controller, ctrlPath);
                _tempAssets.Add(ctrlPath);
            }

            // Build the override controller wrapping the merged base.
            var newOverride = new AnimatorOverrideController(controller);

            // Carry forward any clip overrides the user had in the previous controller,
            // skipping clips that no longer exist in the new base.
            if (avatar.overrides != null)
            {
                var existing = new List<KeyValuePair<AnimationClip, AnimationClip>>();
                avatar.overrides.GetOverrides(existing);
                var toApply = existing.Where(kv => kv.Value != null).ToList();
                if (toApply.Count > 0)
                {
                    try { newOverride.ApplyOverrides(toApply); }
                    catch (System.Exception e)
                    {
                        Debug.LogWarning($"[MA-CVR] Could not carry forward some clip overrides: {e.Message}");
                    }
                }
            }

            // MUST save as an asset — Unity cannot serialise an in-memory
            // AnimatorOverrideController, so SaveChangesToAsset() would null it.
            var overridePath = $"{TempFolder}/MA_CVR_Override_{GUID.Generate()}.overrideController";
            AssetDatabase.CreateAsset(newOverride, overridePath);
            _tempAssets.Add(overridePath);

            avatar.overrides = AssetDatabase.LoadAssetAtPath<AnimatorOverrideController>(overridePath);

            // Keep the editor-only AAS base controller in sync so CCK's
            // "Create AAS Controller" button can re-generate cleanly next time.
