    ...
    internal static class CVRMABoneProxyPass
    {
        internal static void Run(GameObject avatarRoot)
        {
            ...
            foreach (var proxy in proxies)
            {
                ...
                var target = proxy.ResolveTarget(avatarAnimator);
                if (target == null)
                {
                    Debug.LogWarning(
                        $"[MA-CVR] BoneProxy on '{proxy.gameObject.name}': could not resolve " +
                        $"bone '{proxy.boneReference}'" +
                        (string.IsNullOrEmpty(proxy.subPath) ? "" : $" + subPath '{proxy.subPath}'") +
                        " — is the avatar humanoid and the bone mapped? Skipped.", proxy);
                    continue;
                }
